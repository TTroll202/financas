...
document.getElementById('formGastos').addEventListener('submit', function (e) {
  ...
  const valorString = document.getElementById('valorGasto').value.trim();
  ...
  alert(
    `Gasto adicionado: ${descricao}, R$ ${valor.toFixed(2)} no banco ${banco}`,
  );
  this.reset();
});
...
document.getElementById('formLucros').addEventListener('submit', function (e) {
  ...
  const valorString = document.getElementById('valorLucro').value.trim();
  const valor = parseFloat(valorString);
  ...
  alert(
    `Lucro adicionado: ${descricao}, R$ ${valor.toFixed(2)} no banco ${banco}`,
  );
  this.reset();
});
...
function atualizarDashboard() {
  ...
  document.getElementById('gastosTotais').textContent = totalGastos.toFixed(2);
  document.getElementById('lucrosTotais').textContent = totalLucros.toFixed(2);
...
function atualizarRelatorios() {
  ...
  gastos.forEach((gasto) => {
    relatoriosContainer.innerHTML += `<li>${
      gasto.descricao
    }: R$ ${gasto.valor.toFixed(2)} (Banco: ${gasto.banco})</li>`;
  });
  ...
  lucros.forEach((lucro) => {
    relatoriosContainer.innerHTML += `<li>${
      lucro.descricao
    }: R$ ${lucro.valor.toFixed(2)} (Banco: ${lucro.banco})</li>`;
  });
  relatoriosContainer.innerHTML += `</ul>`;
}
